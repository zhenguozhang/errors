...
var (
	unknownCoder defaultCoder = defaultCoder{1, http.StatusInternalServerError, "An internal server error occurred", "http://github.com/zhenguozhang/errors/README.md"}
)
...
type Coder interface {
	...
	// String is external (user) facing error text.
	String() string
...
func Register(coder Coder) {
	if coder.Code() == 0 {
		panic("code `0` is reserved by `github.com/zhenguozhang/errors` as unknownCode error code")
	}
...
func MustRegister(coder Coder) {
	if coder.Code() == 0 {
		panic("code '0' is reserved by 'github.com/zhenguozhang/errors' as ErrUnknown error code")
	}
...
// IsCode check whether the error is the specified code.
